...
mixed_model <- lmer(connectance ~ literature +
                      molecular +
                      natural_history + 
                      paper_1_year +
                      (1|link.citation), 
...
# make partial regressions
partial_regressions <- plot_model(reduced_model_2, type = "eff", show.data = TRUE)

# make them pretty
lit_plot <- partial_regressions[[1]] +
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 15)
        ) +
  labs(title = "", x = "Literature", y = "Connectance")

mlc_plot <- partial_regressions[[2]] +
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 15)
        ) +
  labs(title = "", x = "Molecular", y = "")

nathist_plot <- partial_regressions[[3]] +
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 15)
        ) +
  labs(title = "", x = "Natural History", y = "Connectance")

year_plot <- partial_regressions[[4]] +
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 15)
        ) +
  labs(title = "", x = "Year Published", y = "")

# mash them all together
plot_grid(lit_plot, mlc_plot, nathist_plot, year_plot)
